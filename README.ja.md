...
スターター一式を新しいプロジェクトにコピーし、同梱のプロダクト定義を自分のものに書き換えてください。品質ゲート(lefthook による pre-commit での format / lint / typecheck / 関連テストと、pre-push での全体 typecheck / テスト)は最初から組み込み済みです。
