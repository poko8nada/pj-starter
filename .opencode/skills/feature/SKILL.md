...
6. **Co-append the reason on important decisions** — new work units and `stack` / `roadmap` / `look` changes ship with `set <key>.why '{"why":"…","whyNot":"…"}'` (`whyNot` optional) in the same invocation, so the reason lands atomically with the value it explains. Trivial edits need no reason.
