...
4. **Append** — `set <key> '<full definition>'`; rebuild injects `"status": "planned"`.
...
Route or definition changes on existing components are deep-key sets with full-value assertions. Keep statuses truthful: when new work on a committed component is agreed, assert `<key>.status = "ready"` immediately.
