...
4. **Append** — `set <key> '<full definition>'`; rebuild injects `"stage": "planned"`.
...
Route or definition changes on existing components are deep-key sets with full-value assertions. Keep stages truthful: when new work on a committed component is agreed, assert `<key>.stage = "ready"` immediately.
