...
// CLI 引数をパースする
// 値付きフラグの値を取り出す。欠落（次トークンなし／次がフラグ）を有料呼び出しに流さない
function take(flag, value) {
...
function parseArgs(argv) {
  ...
  for (let i = 0; i < argv.length; i++) {
    ...
    switch (flag) {
      case '--input':
        args.input = take(flag, value);
        i++;
        break;
      case '--style':
        args.style = take(flag, value);
        i++;
        break;
      case '--model':
        args.model = take(flag, value);
        i++;
        break;
      case '--prompt':
        args.prompt = take(flag, value);
        i++;
        break;
      case '--n':
        args.n = parsePositiveInt(flag, take(flag, value));
        i++;
        break;
      case '--aspect-ratio':
        args.aspectRatio = take(flag, value);
        i++;
        break;
      case '--resolution':
        args.resolution = take(flag, value);
        i++;
        break;
      case '--output-format':
        args.outputFormat = take(flag, value);
        i++;
        break;
      case '--out':
        args.out = take(flag, value);
        i++;
        break;
      default:
